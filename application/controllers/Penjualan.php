...
class Penjualan extends CI_Controller {
  ...
  public function lihatpenjualan() {
  	...
  	if($this->input->get('formsubmit')) {
		...
		$data['penjualan'] = $this->penjualan_model->getAll('', '(penjualan.nomor_nota LIKE "%'.$this->input->get('nomor_nota').'%" OR pelanggan.kode LIKE "%'.$this->input->get('nomor_nota').'%" OR pelanggan.nama LIKE "%'.$this->input->get('nomor_nota').'%" OR gudang.nama LIKE "%'.$this->input->get('nomor_nota').'%") AND (penjualan.tanggal_jual >= "'.$dtanggal.'" AND penjualan.tanggal_jual <= "'.$duntil.'" )');		  	
  	} else {
  		$data['penjualan'] = $this->penjualan_model->getAll('', 'penjualan.tanggal_jual >= "'.date('Y-m-01').'" AND penjualan.tanggal_jual <= "'.date('Y-m-d').'" ');
  	}

  	$data['totalharga'] =  array();
		  	foreach($data['penjualan'] as $value) {
		  		$data['totalharga'][] = $this->penjualan_model->getTotalHarga($value->nomor_nota);
		  	}
  ...
  Toast.fire({
    icon: 'success',
    title: 'Nota penjualan berhasil dihapus.'
  });
    	";
    	}
    ...
    $('#untiltanggal').on('change.datetimepicker', ({date, oldDate}) => {              
  	...
  	$this->load->view('v_header', $data);
		...
		$this->load->view('penjualan/v_lihat_penjualan', $data);
		$this->load->view('v_footer', $data);
  }

  public function lihatreturpenjualan() {
  	$data = array();
  	...
  	if($this->input->get('formsubmit')) {
		...
		$data['penjualan'] = $this->penjualan_model->getAllRetur('', '(retur_penjualan.nomor_nota LIKE "%'.$this->input->get('nomor_nota').'%" OR pelanggan.kode LIKE "%'.$this->input->get('nomor_nota').'%" OR pelanggan.nama LIKE "%'.$this->input->get('nomor_nota').'%" OR gudang.nama LIKE "%'.$this->input->get('nomor_nota').'%") AND (retur_penjualan.tanggal >= "'.$dtanggal.'" AND retur_penjualan.tanggal <= "'.$duntil.'" )');		  	
  	} else {
  		$data['penjualan'] = $this->penjualan_model->getAllRetur('', 'retur_penjualan.tanggal >= "'.date('Y-m-01').'" AND retur_penjualan.tanggal <= "'.date('Y-m-d').'" ');
  	}

  	$data['totalharga'] =  array();
		  	foreach($data['penjualan'] as $value) {
		  		$data['totalharga'][] = $this->penjualan_model->getTotalHargaRetur($value->nomor_nota);
		  	}
  ...
  Toast.fire({
    icon: 'success',
    title: 'Nota retur penjualan berhasil dihapus.'
  });
    	";
    	}
    ...
    $('#untiltanggal').on('change.datetimepicker', ({date, oldDate}) => {              
  	...
  	$this->load->view('v_header', $data);
		$this->load->view('v_menu', $data);
		$this->load->view('penjualan/v_lihat_penjualan_retur', $data);
		$this->load->view('v_footer', $data);
  }
  ...
  public function tambah() {
  	$data = array();
  		...
  		if($this->input->post('nomor_nota')) {
			...
			if($this->input->post('editmode')) {
	  		...
	  		$this->penjualan_model->doUpdate($this->input->post('nomor_nota'), $dataupdate, $databarang);
	  		$this->session->set_flashdata('update_msg', true);
	  		redirect('penjualan/tambah?id='.$this->input->post('nomor_nota'));
	  		...
	  		$this->penjualan_model->doInsert($datainsert, $databarang);
  	...
  	if($this->input->get('id')) {
  		$data['editnota'] = $this->penjualan_model->getAll($this->input->get('id'));
  		$data['editpelanggan'] = $this->pelanggan_model->getAll($data['editnota'][0]->idpelanggan);
  		if(count($data['editnota']) == 0) { redirect('penjualan/tambah'); }
  		$data['notabarang'] = $this->penjualan_model->getAllBarang($this->input->get('id'));
  	...
  	$data['js'] .= "
	    ...
	    $('.tanggaljual, .jatuhtempo').on('keypress', function(e) {
			var keyCode = (e.keyCode ? e.keyCode : e.which);
			if (keyCode >= 47 && keyCode <= 57 )  {
		    	if($(this).val().length ==10) {
		    		e.preventDefault();
		...
		$('.tanggaljual, .jatuhtempo').on('focus', function() {
			var a = $(this).val().split(' ');
			var b = months.indexOf(a[1]) + 1;
			if(b < 10) {
				$(this).val(a[0] + '/0' + b + '/' + a[2]);	
		...
		$('.tanggaljual, .jatuhtempo').on('change', function() {
			var tgljual = $(this).val();
			var y = tgljual.substr(6,10);
			var m = parseInt(tgljual.substr(3,5));
			var d = tgljual.substr(0,2);
		...
		$('#tanggaljual').on('keyup', function(e) {
			if(e.keyCode == 13) { 
				console.log('check');
				var tgljual = $('.tanggaljual').val();
				console.log(tgljual);
				if(tgljual.split('/').length > 1) {
					var y = tgljual.substr(6,10);
					var m = parseInt(tgljual.substr(3,5));
					var d = tgljual.substr(0,2);
					$('.tanggaljual').val(d + ' ' + months[m-1] + ' ' + y);
					console.log($('.tanggaljual').val());
				}
		...
		$('#jatuhtempo').on('keyup', function(e) {
			if(e.keyCode == 13) { 
				console.log('check');
				var tgljual = $('.jatuhtempo').val();
				console.log(tgljual);
				if(tgljual.split('/').length > 1) {
					var y = tgljual.substr(6,10);
					var m = parseInt(tgljual.substr(3,5));
					var d = tgljual.substr(0,2);
					$('.jatuhtempo').val(d + ' ' + months[m-1] + ' ' + y);
					console.log($('.jatuhtempo').val());
				}
		...
		$('.tanggaljual, .jatuhtempo').on('blur', function() {
			var tgljual = $(this).val();
			if($(this).val().split('/').length > 1) {
				var y = tgljual.substr(6,10);
				var m = parseInt(tgljual.substr(3,5));
  	...
  	$('#btnsubmit').on('click', function(e) {
  		if($('#formtmabahpenjualan').valid()) {
	  		if(barangs.length == 0) {
	  			...
	  			e.preventDefault();
	  			// CEK KODE PELANGGAN
				$.post('".base_url('penjualan/searchpelanggankode')."', { nomor_nota : $('#namapelanggan').val() },function(data) {
  	...
  	$('#loadlink').on('click', function(e) {
  		...
  		if($('#nomor_nota').val().trim() != '') {
	  		$.post('".base_url('penjualan/searchnota')."', { nomor_nota: $('#nomor_nota').val() }, function(data) {
	  			var json = JSON.parse(data);
				if(json['data'].length > 0) {
					window.location.replace('".base_url('penjualan/tambah?id=')."' + $('#nomor_nota').val());
...
$('#btnadd').on('click', function() {
  ...
  Toast.fire({
    icon: 'success',
    title: 'Data penjualan berhasil ditambahkan.'
  });
    	";
    	}
  ...
  Toast.fire({
    icon: 'success',
    title: 'Data penjualan berhasil diperbaharui.'
  });
    	";
    	}
  ...
  Toast.fire({
    ...
    $data['js'] .= "
		...
		$('#carinota').on('keyup change keydown focus', function() {
  		var term = $(this).val();
  		if(term.trim() != '') {
	  		$.post('".base_url('penjualan/searchnotafree')."', { cari:term }, function(data) {
		  			var json = JSON.parse(data);
		  			if(json['data'].length > 0) {
		  				var tableserupa = '';
						var serupa=0;
						for(var i =0; i < json['data'].length; i++) {
							var tgljual = json['data'][i].tanggal_jual;
							...
							tableserupa += '<tr>';
							tableserupa += '<td>J' + json['data'][i].nomor_nota.toUpperCase() + '</td>';
							...
							tableserupa += '<td><a href=\'".base_url('penjualan/tambah?id=')."' + json['data'][i].nomor_nota + '\'   class=\'btn btnpilih btn-primary btn-sm\'>PILIH</a></td>';
							tableserupa += '</tr>';
							serupa++;
						}
    ...
    ";
  	...
  	$this->load->view('v_header', $data);
		$this->load->view('v_menu', $data);
		$this->load->view('penjualan/v_tambah_penjualan', $data);
		$this->load->view('v_footer', $data);
  }
  ...
  public function tambahretur() {
  	...
  	if($this->input->post('nomor_nota')) {
			...
			if($this->input->post('editmode')) {
	  		...
	  		$this->penjualan_model->doUpdateRetur($this->input->post('nomor_nota'), $dataupdate, $databarang);
	  		$this->session->set_flashdata('update_msg', true);
	  		redirect('penjualan/tambahretur?id='.$this->input->post('nomor_nota'));
	  		...
	  		$this->penjualan_model->doInsertRetur($datainsert, $databarang);
	  		$this->session->set_flashdata('insert_msg', true);
	  		redirect('penjualan/tambahretur');
			}		  		
  	...
  	if($this->input->get('id')) {
  		$data['editnota'] = $this->penjualan_model->getAllRetur($this->input->get('id'));
  		$data['editpelanggan'] = $this->pelanggan_model->getAll($data['editnota'][0]->idpelanggan);
  		if(count($data['editnota']) == 0) { redirect('penjualan/tambahretur'); }
  		$data['notabarang'] = $this->penjualan_model->getAllBarangRetur($this->input->get('id'));
  	...
  	$('#loadlink').on('click', function(e) {
  		e.preventDefault();
  		if($('#nomor_nota').val().trim() != '') {
	  		$.post('".base_url('penjualan/searchreturnota')."', { nomor_nota: $('#nomor_nota').val() }, function(data) {
	  			var json = JSON.parse(data);
				if(json['data'].length > 0) {
					window.location.replace('".base_url('penjualan/tambahretur?id=')."' + $('#nomor_nota').val());
				} else {
					alert('Nomor nota tidak ditemukan');
				}
	  		});
...
$('#btnadd').on('click', function() {
  ...
  Toast.fire({
    icon: 'success',
    title: 'Data retur penjualan berhasil ditambahkan.'
  });
    	";
    	}
  ...
  Toast.fire({
    icon: 'success',
    title: 'Data retur penjualan berhasil diperbaharui.'
  });
    	";
    	}
  ...
  Toast.fire({
    ...
    $data['js'] .= "
		...
		$('#carinota').on('keyup change keydown focus', function() {
  		var term = $(this).val();
  		if(term.trim() != '') {
	  		$.post('".base_url('penjualan/searchnotareturfree')."', { cari:term }, function(data) {
		  			...
		  			if(json['data'].length > 0) {
						...
						for(var i =0; i < json['data'].length; i++) {
							...
							tableserupa += '<tr>';
							tableserupa += '<td>RJ' + json['data'][i].nomor_nota.toUpperCase() + '</td>';
							tableserupa += '<td>' + json['data'][i].pelanggannama.toUpperCase() + '</td>';
							tableserupa += '<td>' + json['data'][i].gudangnama.toUpperCase() + '</td>';
							tableserupa += '<td>' + d+' '+months[m-1] +' '+y+ '</td>';
							tableserupa += '<td><a href=\'".base_url('penjualan/tambahretur?id=')."' + json['data'][i].nomor_nota + '\'   class=\'btn btnpilih btn-primary btn-sm\'>PILIH</a></td>';
							tableserupa += '</tr>';
							serupa++;
						}
    ...
    ";
	  		...
	  		$.post('".base_url('penjualan/searchpelanggan')."', { cari:term }, function(data) {
	  			...
	  			if(json['data'].length > 0) {
						...
						for(var i =0; i < json['data'].length; i++) {
							...
							tableserupa += '<td>' + json['data'][i].telepon.toUpperCase() + '</td>';
							tableserupa += '<td>' + json['data'][i].keterangan.toUpperCase() + '</td>';
							tableserupa += '<td><button type=\'button\' pilihnama=\'' + json['data'][i].nama + '\' pilihcontact=\'' + json['data'][i].contact_person + '\' pilihtelepon=\'' + json['data'][i].telepon + '\' pilihid=\'' + json['data'][i].idpelanggan + '\' class=\'btn btnpilih btn-primary btn-sm\'>PILIH</button></td>';
							tableserupa += '</tr>';
							serupa++;
						}
  	...
  	$this->load->view('v_header', $data);
		$this->load->view('v_menu', $data);
		$this->load->view('penjualan/v_tambah_retur', $data);
		$this->load->view('v_footer', $data);
  }
	...
	public function searchnota() {
		$hasil = $this->penjualan_model->getAll($this->input->post('nomor_nota'));
		echo json_encode(array('data' => $hasil));
	}
	...
	public function searchnotafree() {
		$hasil = $this->penjualan_model->getAllFree( $this->input->post('cari'), ' penjualan.status="active"');
		echo json_encode(array('data' => $hasil));
	}

	public function searchnotareturfree() {
		$hasil = $this->penjualan_model->getAllReturFree( $this->input->post('cari'), ' retur_penjualan.status="active"');
		echo json_encode(array('data' => $hasil));
	}	

	public function del($nomor_nota) {
		$this->penjualan_model->doDel($nomor_nota);
		$this->session->set_flashdata('del_msg', true);

		redirect('penjualan/lihatpenjualan');
	}

	public function delRetur($nomor_nota) {
		$this->penjualan_model->doDelRetur($nomor_nota);
		$this->session->set_flashdata('del_msg', true);

		redirect('penjualan/lihatreturpenjualan');
	}
}
